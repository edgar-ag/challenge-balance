...
func GetBalance(txns []*models.Transaction) (*models.TemplateData, error) {
	...
	totalDebit := getTotalOf(debitTxns)
	totalCredit := getTotalOf(creditTxns)
	totalBalance := totalDebit + totalCredit
	averageDebitAmount := totalDebit / float32(len(debitTxns))
	averageCreditAmount := totalCredit / float32(len(creditTxns))
...
func getTotalOf(txns []float32) float32 {
	total := 0.0
	for _, txn := range txns {
		total += float64(txn)
	}
